...
| 指标 | 零样本 | 微调后 | 变化 |
|------|--------|--------|------|
| Precision | 0.216 | **0.220** | +0.004 |
| Recall | 0.306 | **0.324** | +0.018 |
| F1 | 0.167 | **0.173** | +0.006 |
| Mean IoU | 0.315 | **0.329** | +0.014 |
| TP 匹配 | 74 | **86** | **+16%** |
| 预测 bbox | 685 | **590** | -14%（更精炼）|
| GT bbox | 968 | 968 | - |
| Auto Score | 0.446 | 0.447 | +0.001 |

**微调训练详情**：
- 样本：223 个有异常标注的训练集图片
- 参数：LoRA rank=16, alpha=32, 目标层=q/k/v/o_proj
- Loss：10.4 → 3.0（3 个 epoch，收敛良好）
- 图片缩至 800px + gradient checkpointing

**Qwen3.5-27B 结论**：确认多模态，但 4-bit 仍需 46GB+，48GB 单卡 OOM，当前不可用
