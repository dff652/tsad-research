...
| 策略 | 预测均分 | 通过率 | 特点 |
|------|---------|--------|------|
| Timer (单独) | 0.724 | 88.2% | SOTA，精细检测 |
| Weighted Ensemble | 0.717 | 85.2% | Timer+统计加权，接近 SOTA |
| Voting (≥2) | 0.617 | 70.4% | 折中但过于保守 |
| ADTK+HBOS | 0.586 | 66.7% | 旧 baseline |
| Cascade | 0.443 | 37.0% | 统计过滤太多假阳性 |

**关键洞察**：Timer 与统计方法检测到的是不同维度的异常。Timer 基于预测残差（时序上下文相关），统计方法基于数值分布（上下文无关）。最佳策略是以 Timer 为主、统计方法为辅。

### 4.4 自动评分器

基于规则的自动评分器预测 Timer 的人工均分误差仅 0.052（预测 0.724 vs 实际 0.672）。核心规则：
- 零异常 = 差评（漏检）
- 异常率 > 50% = 差评（乱叫）
- 异常率 0.1%-15% 区间 = 基础高分
- 精细簇数量 > 50 = 加分

### 4.5 Timer vs Sundial 等价性

在标准降采样（M4→10000 点）+ MAD 阈值（k=3.5）的流程下，Timer-84M 和 Sundial-128M 产生**100% 相同的异常检测结果**。差异被降采样和离散化阈值抹平。

**区分两者需要**：更精细的降采样、连续异常评分、或不同的阈值参数。

### 4.6 参数调优实验

threshold_k 扫描结果（10 个代表性点位）：

| threshold_k | 平均异常率 | 特点 |
|-------------|-----------|------|
| 2.5 | 9.66% | 敏感，更多检出 |
| 3.0 | 7.89% | 略敏感 |
| **3.5 (默认)** | **6.88%** | **平衡** |
...
**结论**：阈值变化不显著影响检测质量评分——真正的差异在于模型本身的预测能力，而非后处理阈值。

### 4.7 Timer 检测 vs 人工评分的深层关系

- Timer 推理特征与其人工评分相关性弱（max r=-0.24）
- **高评分场景**：ADTK 异常率高（数据有明显异常）+ Timer 检测精炼（簇少但准）
- **低评分场景**：ADTK 异常率低（数据相对平静）+ Timer 簇多（可能过度检测）
- Timer 推理异常率与 ADTK 异常率高度相关（r=0.69），但粒度完全不同
...
- 436 个全量点位推理：~7 小时
- 统计方法（IForest/3-Sigma/MAD/IQR）100 点位：~10 分钟

---

## 五、综合排名（里程碑 1-3 结论）

| 排名 | 算法 | 预测均分 | 通过率 | 良好率 | 方法类型 |
|------|------|---------|--------|--------|---------|
| **1** | **Timer-84M** | **0.764** | **94.9%** | **87.9%** | TSFM |
| 2 | Weighted Ensemble | 0.717 | 85.2% | 74.1% | 集成 |
| 3 | IForest | 0.670 | 77.8% | 50.5% | 统计 |
| 4 | Voting Ensemble | 0.617 | 70.4% | 33.3% | 集成 |
| 5 | ADTK+HBOS | 0.586 | 66.7% | 6.1% | 统计 |
| 6 | IQR | 0.529 | 55.6% | 36.4% | 统计 |
| 7 | MAD | 0.498 | 47.5% | 30.3% | 统计 |
| 8 | Cascade Ensemble | 0.443 | 37.0% | 11.1% | 集成 |
| 9 | 3-Sigma | 0.436 | 40.4% | 21.2% | 统计 |

**实际人工 Timer 均分**: 0.672 (5 位专家评分)
...
### 里程碑 4：全量泛化
- 在 436 全量点位上运行 Timer（预计 7 小时）
- 构建按传感器类型分层的差异化检测策略

### 里程碑 5：业务收敛
- 将 Timer 最佳配置封装为生产级 API
- 集成到 `ts-iteration-loop` 主干
- 技术白皮书撰写
